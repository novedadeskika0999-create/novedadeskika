...
async function cargarDesdeDrive() {
    ...
    try {
        ...
        if (typeof actualizarUICompleta === 'function') {
            ...
            if (typeof actualizarListaProductos === 'function') actualizarListaProductos();
            if (typeof actualizarTablaCompradores === 'function') actualizarTablaCompradores();
    ...
    if (token && usuario) {
        try {
            ...
            if (test.ok) {
                ...
                await cargarDesdeDrive();
                iniciarSyncAutomatico();
                // Forzar re-render por si la UI ya estaba montada
                setTimeout(() => {
                    if (typeof actualizarUICompleta === 'function') actualizarUICompleta();
                }, 800);
                return true;
