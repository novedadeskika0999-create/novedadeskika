...
const FIREBASE_CONFIG = {
    ...
    projectId: "novedadeskika-9601a",// sync.js — Firestore tiempo real con PIN
const FIREBASE_CONFIG = {
    apiKey: "AIzaSyCbhpjJ6gkK4CTc7c9C83alJHPSzTFzQ08",
    authDomain: "novedadeskika-9601a.firebaseapp.com",
...
const CORREOS_AUTORIZADOS = ['novedadeskika0999@gmail.com','myk1xk@gmail.com','epro9749@gmail.com','mikyy0811@gmail.com'];
const PINES_ACCESO = {'1111':'novedadeskika0999@gmail.com','2222':'myk1xk@gmail.com','3333':'epro9749@gmail.com','4444':'mikyy0811@gmail.com'};

let _db = null;
let _usuarioActual = null;
let _unsubscribe = null;
let _cargadoDeFirestore = false;
let _miUltimoGuardado = null; // ID único de mi último guardado
...
function _escuchar() {
    ...
    _unsubscribe = _db.collection('datos').doc('principal')
        .onSnapshot({ includeMetadataChanges: false }, doc => {
            ...
            const d = doc.data();
            // Si este snapshot es consecuencia de MI propio guardado, ignorarlo
            if (d.editadoPor === _miUltimoGuardado) {
                _setSyncStatus('ok');
                return;
            }
...
async function _guardar() {
    ...
    // ID único para este guardado — para ignorar el eco del snapshot
    const id = _usuarioActual + '_' + Date.now();
    _miUltimoGuardado = id;
    _setSyncStatus('saving');
    try {
        await _db.collection('datos').doc('principal').set({
            compras, logistica, participantes, rifaCompras,
            resumenMarcado, resumenManualOverrides, manualMarkCount,
            cuentasGeneradas,
        ...
        _setSyncStatus('error');
        // Si falló, resetear ID para que el próximo snapshot no sea ignorado
...
// API pública — llamada desde todos los archivos
let _debounce = null;
function guardarEnDriveConDebounce() {
    if (!_cargadoDeFirestore) return;
    clearTimeout(_debounce);
    _debounce = setTimeout(() => _guardar(), 300);
}
...
function _setSyncStatus(e) {
    ...
    if (!el) return;
    const m = {ok:'<i class="fas fa-cloud"></i> Sincronizado',saving:'<i class="fas fa-cloud-upload-alt"></i> Guardando...',syncing:'<i class="fas fa-sync fa-spin"></i> Conectando...',error:'<i class="fas fa-exclamation-triangle"></i> Sin conexión'};
